...
- [x] Chart 컴포넌트를 생성하고 선택된 지표에 맞춰 Multi-Layer 그래프 렌더링
- [x] 환율(4자리)과 지수(2~3자리) 등 단위가 다른 데이터를 위한 Dual Y-Axis 설계
- [x] Zustand와 연동하여 체크박스에 따른 동적 차트 업데이트
- [x] 기간 필터링(Temporal Resolution) 상태에 따른 차트 데이터 재호출
